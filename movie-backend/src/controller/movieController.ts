...
// ✅ Generic handler for fetching lists
const handleList = (fetchFn: () => Promise<any>, message: string) =>
  async (_: Request, res: Response) => {
    const data = await fetchFn();
    res.json({ success: true, message, data });
  };

// ✅ Generic handler for ID-based fetches
const handleById = (
...
export const fetchTrendingMovies = handleList(getPopularMovies, POPULAR_MOVIES_FETCHED);
...
export const handleSearchMovies = async (req: Request, res: Response) => {
  ...
  const data = await searchFromTMDb(query);
